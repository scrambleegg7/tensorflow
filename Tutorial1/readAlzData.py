...
def readDemographicdMaster():
    ...
    diag_ = df["diagnosis"].values.tolist()
    
    idx = np.arange(len(diag_))
    
    diag_ = np.array(diag_)
    validation = diag_[np.isnan(diag_)] 
    training = diag_[~ np.isnan(diag_)]
    
    valid_ =  len(validation)
    
    valid_idx = idx[-valid_:]
    #print valid_idx    
    
    training_idx = idx[:len(training)]
    
    #print training_idx
    
    diagnosis_flag =LabelBinarizer().fit_transform(training)
...
def alzDataDir():
    ...
    #plot_gallary_alz(first)

def plot_gallary_alz(images):
...
def process():
    ...
    print "-- just testing purposes - MNIST -- : "
...
def main():
    ...
    print trainingOneHot.shape,validOneHot.shape,len(train_orig),len(valid_orig)
    """
    files = alzDataDir()    
    ...
    convProc(train_image,diagnosis)
    """
